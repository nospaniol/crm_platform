...
var KTInnovativeGeneral = function () {
    ...
    var viewInnovativeForm = function () {
        ...
        $('#kt_innovative_search').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: 'http://127.0.0.1:800/tollspending/filter/monthly/by/' + client + '/' + month + '/' + year + '/' + filter,
                success: function (res) {
                    ...
                    if (title === "success") {
                        ...
                        search_results.forEach((item) => {
                            var col = '<td scope="row"> <i class="flaticon2-correct"></i> ' + item.title + '</td>';
                            if (filter === 'licensePlate') {
                                col = '<td scope="row"> <i class="flaticon2-correct"></i> ' + item.vehicle.licensePlate + '</td>';
                            }
                            $('#data_info').append(
                                    '<tr>' +
                                    col +
        ...
        $('#kt_innovative_department_search').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: 'http://127.0.0.1:800/tollspending/filter/department/monthly/by/' + client + '/' + month + '/' + year + '/' + filter,
                success: function (res) {
                    var title = res["title"];
                    var message = res["message"];
                    var pageTitle = res["pageTitle"];
        ...
        $('#kt_innovative_annual_search').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                url: 'http://127.0.0.1:800/tollspending/filter/annual/view/by/' + client + '/' + year + '/' + filter,
                success: function (res) {
                    var title = res["title"];
                    var message = res["message"];
                    var pageTitle = res["pageTitle"];
