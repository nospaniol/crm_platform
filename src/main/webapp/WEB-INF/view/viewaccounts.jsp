                                ...
                                <c:choose>
                                    <c:when test="${data.size() > 0 }">
                                        <c:forEach var="account" items="${data}">
                                            <tr>               
                                                <td scope="row"> <i class="flaticon2-correct"></i> ${account.clientProfile.companyName}</td>
                                                <td>${account.amount}</td>
                                                <td>
                                                    <a href="<c:url value='http://127.0.0.1:800/account/view/department/${account.clientProfile.clientProfileId}' />">
                                   ...
                                   <!-- Showing ${number+1} <b>to</b> ${size+1} of --> <!-- Total  Entries ::  <b>  ${totalElements} </b> -->
                                    <ul class="pagination"> Page  ::     
                                        <c:forEach begin="0" end="${totalPages-1}" var="page">
                                            <li class="page-item"> 
                                                <a href="<c:url value='http://127.0.0.1:800/account/view_accounts?page=${page}&size=${size}' />" class="page-link">pg. ${page+1}</a>
                                            </li>
                                        </c:forEach>
                                    </ul>
                                </div>
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <p>COMPANY : ${account.clientProfile.companyName}</p>
                        </div>
                ...
                function depositAmount${account.accountId}() {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: 'http://127.0.0.1:800/account/deposit/' + amount + '/' + accountId,
                ...
                function subtractAmount${account.accountId}() {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: 'http://127.0.0.1:800/account/credit/' + amount + '/' + accountId,
                                success: function (res) {
                                    var title = res["title"];
                                    var message = res["message"];
                                    if (title === "fail") {
