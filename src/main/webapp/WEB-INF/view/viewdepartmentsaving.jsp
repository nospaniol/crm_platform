                                ...
                                <c:choose>
                                    <c:when test="${data.size() > 0 }">
                                        <c:forEach var="saving" items="${data}">
                                            <tr>               
                                                <td>${saving.department.departmentName}</td>
                                                <td>${saving.amount}</td>
                                                <td>
                                   ...
                                   <!-- Showing ${number+1} <b>to</b> ${size+1} of --> <!-- Total  Entries ::  <b>  ${totalElements} </b> -->
                                    <ul class="pagination"> Page  ::     
                                        <c:forEach begin="0" end="${totalPages-1}" var="page">
                                            <li class="page-item"> 
                                                <a href="<c:url value='http://127.0.0.1:800/saving/view/department/${clientId}?page=${page}&size=${size}' />" class="page-link">pg. ${page+1}</a>
                                            </li>
                                        </c:forEach>
                                    </ul>
                                </div>
...
<script  src="<c:url value="/resources/assets/js/nshome/pages/wizard/manage-department-saving.js?v=1001" />"   type="text/javascript"></script>
<c:choose>
    <c:when test="${data.size() > 0 }">
        <c:forEach var="saving" items="${data}">                          
                ...
                <div class="modal-dialog">
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <p>Department : ${saving.department.departmentName}</p>
                        </div>
                ...
                function depositAmount${saving.savingId}() {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: 'http://127.0.0.1:800/saving/department/deposit/' + amount + '/' + savingId,
                ...
                function subtractAmount${saving.savingId}() {
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        if (result.value) {
                            $.ajax({
                                type: "POST",
                                url: 'http://127.0.0.1:800/saving/department/credit/' + amount + '/' + savingId,
                                success: function (res) {
                                    var title = res["title"];
                                    var message = res["message"];
                                    if (title === "fail") {
